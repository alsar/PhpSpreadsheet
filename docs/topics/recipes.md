...
$spreadsheet->getActiveSheet()->getStyle('D1')
    ->getNumberFormat()
    ->setFormatCode(\PhpSpreadsheet\Style\NumberFormat::FORMAT_DATE_YYYYMMDDSLASH);

// PHP-time (Unix time)
$time = gmmktime(0,0,0,12,31,2008); // int(1230681600)
$spreadsheet->getActiveSheet()
    ->setCellValue('D1', \PhpSpreadsheet\Shared\Date::PHPToExcel($time));
$spreadsheet->getActiveSheet()->getStyle('D1')
    ->getNumberFormat()
    ->setFormatCode(\PhpSpreadsheet\Style\NumberFormat::FORMAT_DATE_YYYYMMDDSLASH);
...
The `\PhpSpreadsheet\Shared\Date::PHPToExcel()` method will also
...
`\PhpSpreadsheet\Style\NumberFormat` provides a number of
...
```php
use PhpSpreadsheet\Style\NumberFormat\Wizard\Date as DateWizard;
use PhpSpreadsheet\Style\NumberFormat\Wizard\Time as TimeWizard;
use PhpSpreadsheet\Style\NumberFormat\Wizard\DateTime as DateTimeWizard;
...
$spreadsheet->getActiveSheet()
    ->setCellValueExplicit(
        ...
        \PhpSpreadsheet\Cell\DataType::TYPE_STRING
    );
```
...
```php
$formula = $spreadsheet->getActiveSheet()->getCell('B8')->getValue();
$translatedFormula = \PhpSpreadsheet\Calculation\Calculation::getInstance()->translateFormulaToLocale($formula);
```
...
$internalFormula = \PhpSpreadsheet\Calculation\Calculation::getInstance()->translateFormulaToEnglish($formula);
...
$spreadsheet->getActiveSheet()->getCell('A1')
    ->setValueExplicit(
        '25',
        \PhpSpreadsheet\Cell\DataType::TYPE_NUMERIC
    );
```
...
```php
\PhpSpreadsheet\Worksheet\PageSetup::setPaperSizeDefault(
...
```php
\PhpSpreadsheet\Worksheet\PageSetup::setOrientationDefault(
    \PhpSpreadsheet\Worksheet\PageSetup::ORIENTATION_LANDSCAPE
);
```
...
```php
$spreadsheet = \PhpSpreadsheet\IOFactory::load('test.xlsx');
...
$spreadsheet->getActiveSheet()->getHeaderFooter()->addImage($drawing, \PhpSpreadsheet\Worksheet\HeaderFooter::IMAGE_HEADER_LEFT);
```
...
```php
$spreadsheet->getActiveSheet()->setBreak('A10', \PhpSpreadsheet\Worksheet\Worksheet::BREAK_ROW);
```
...
$spreadsheet->getActiveSheet()
    ->setBreak('A10',
        \PhpSpreadsheet\Worksheet\Worksheet::BREAK_ROW,
        \PhpSpreadsheet\Worksheet\Worksheet::BREAK_ROW_MAX_COLUMN);
```

The following line of code sets a print break on column D:

```php
$spreadsheet->getActiveSheet()->setBreak('D10', \PhpSpreadsheet\Worksheet\Worksheet::BREAK_COLUMN);
```
...
$spreadsheet->getActiveSheet()->getStyle('B2')
    ->getFont()->getColor()->setARGB(\PhpSpreadsheet\Style\Color::COLOR_RED);
$spreadsheet->getActiveSheet()->getStyle('B2')
    ->getAlignment()->setHorizontal(\PhpSpreadsheet\Style\Alignment::HORIZONTAL_RIGHT);
$spreadsheet->getActiveSheet()->getStyle('B2')
    ->getBorders()->getTop()->setBorderStyle(\PhpSpreadsheet\Style\Border::BORDER_THICK);
$spreadsheet->getActiveSheet()->getStyle('B2')
    ->getBorders()->getBottom()->setBorderStyle(\PhpSpreadsheet\Style\Border::BORDER_THICK);
$spreadsheet->getActiveSheet()->getStyle('B2')
    ->getBorders()->getLeft()->setBorderStyle(\PhpSpreadsheet\Style\Border::BORDER_THICK);
$spreadsheet->getActiveSheet()->getStyle('B2')
    ->getBorders()->getRight()->setBorderStyle(\PhpSpreadsheet\Style\Border::BORDER_THICK);
$spreadsheet->getActiveSheet()->getStyle('B2')
    ->getFill()->setFillType(\PhpSpreadsheet\Style\Fill::FILL_SOLID);
$spreadsheet->getActiveSheet()->getStyle('B2')
...
$styleArray = [
    ...
    'fill' => [
        'fillType' => \PhpSpreadsheet\Style\Fill::FILL_GRADIENT_LINEAR,
        'rotation' => 90,
        'startColor' => [
            'argb' => 'FFA0A0A0',
        ],
...
$spreadsheet->getActiveSheet()->getStyle('A1')->getNumberFormat()
    ->setFormatCode(\PhpSpreadsheet\Style\NumberFormat::FORMAT_NUMBER_COMMA_SEPARATED1);
```
...
$spreadsheet->getActiveSheet()->getStyle('A1:D4')
    ->getAlignment()->setVertical(\PhpSpreadsheet\Style\Alignment::VERTICAL_TOP);
```
...
$styleArray = [
    'borders' => [
        'outline' => [
            'borderStyle' => \PhpSpreadsheet\Style\Border::BORDER_THICK,
            'color' => ['argb' => 'FFFF0000'],
        ],
    ],
];
...
`\PhpSpreadsheet\Style\Style::applyFromArray()` classes. If the "Maps
...
**\PhpSpreadsheet\Style\Alignment**
...
**\PhpSpreadsheet\Style\Protection**
...
$conditional1->setOperatorType(\PhpSpreadsheet\Style\Conditional::OPERATOR_LESSTHAN);
...
$conditional2->setOperatorType(\PhpSpreadsheet\Style\Conditional::OPERATOR_GREATERTHANOREQUAL);
$conditional2->addCondition('0');
$conditional2->getStyle()->getFont()->getColor()->setARGB(\PhpSpreadsheet\Style\Color::COLOR_GREEN);
...
$spreadsheet->getActiveSheet()->getStyle('B1')
    ->getProtection()
    ->setLocked(\PhpSpreadsheet\Style\Protection::PROTECTION_UNPROTECTED);
```
...
`\PhpSpreadsheet\Worksheet\Drawing`, and assign its properties a
meaningful value:
...
`\PhpSpreadsheet\RichText\RichText` instances. Here''s an example, which
...
```php
$richText = new \PhpSpreadsheet\RichText\RichText();
...
// Define named ranges
$spreadsheet->addNamedRange( new \PhpSpreadsheet\NamedRange('PersonFN', $spreadsheet->getActiveSheet(), '$B$1'));
$spreadsheet->addNamedRange( new \PhpSpreadsheet\NamedRange('PersonLN', $spreadsheet->getActiveSheet(), '$B$2'));
```
...
$spreadsheet->addNamedFormula( new \PhpSpreadsheet\NamedFormula('GERMAN_VAT_RATE', $worksheet, '=16.0%'));
$spreadsheet->addNamedFormula( new \PhpSpreadsheet\NamedFormula('CALCULATED_PRICE', $worksheet, '=$B1*$C1'));
$spreadsheet->addNamedFormula( new \PhpSpreadsheet\NamedFormula('GERMAN_VAT', $worksheet, '=$D1*GERMAN_VAT_RATE'));
$spreadsheet->addNamedFormula( new \PhpSpreadsheet\NamedFormula('TOTAL_INCLUDING_VAT', $worksheet, '=$D1+$E1'));
...
$spreadsheet->addNamedRange( new \PhpSpreadsheet\NamedRange('COLUMN_TOTAL', $worksheet, '=A$2:A$4') );
...
3.  Use the `\PhpSpreadsheet\Writer\*` of your choice, and save
    to `'php://output'`

`\PhpSpreadsheet\Writer\Xlsx` uses temporary storage when
...
```php
$spreadsheet->getActiveSheet()
    ->setSheetState(\PhpSpreadsheet\Worksheet\Worksheet::SHEETSTATE_HIDDEN);
```

Sometimes you may even want the worksheet to be **"very hidden"**. The
available sheet states are :

-   `\PhpSpreadsheet\Worksheet\Worksheet::SHEETSTATE_VISIBLE`
-   `\PhpSpreadsheet\Worksheet\Worksheet::SHEETSTATE_HIDDEN`
-   `\PhpSpreadsheet\Worksheet\Worksheet::SHEETSTATE_VERYHIDDEN`
