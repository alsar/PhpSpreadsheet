...
$dateTimeNow = time();
$excelDateValue = \PhpSpreadsheet\Shared\Date::PHPToExcel( $dateTimeNow );
...
`\PhpSpreadsheet\Cell\IValueBinder` implementation
(\PhpSpreadsheet\Cell\DefaultValueBinder) to determine data
...
`\PhpSpreadsheet\Cell\AdvancedValueBinder` class is available.
...
Alternatively, a `\PhpSpreadsheet\Cell\StringValueBinder` class is available
...
`\PhpSpreadsheet\Cell\IValueBinder` interface or extend the existing
`\PhpSpreadsheet\Cell\DefaultValueBinder` or
`\PhpSpreadsheet\Cell\AdvancedValueBinder` classes.
