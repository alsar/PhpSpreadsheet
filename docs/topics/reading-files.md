...
method of the `\PhpSpreadsheet\IOFactory` class.
...
$inputFileName = './sampleData/example1.xls';
$testAgainstFormats = [
    \PhpSpreadsheet\IOFactory::READER_XLS,
    \PhpSpreadsheet\IOFactory::READER_HTML,
];

/** Load $inputFileName to a Spreadsheet Object  **/
$spreadsheet = \PhpSpreadsheet\IOFactory::load($inputFileName, 0, $testAgainstFormats);
```
...
//    $reader = new \PhpSpreadsheet\Reader\Gnumeric();
...
/**  Identify the type of $inputFileName  **/
$inputFileType = \PhpSpreadsheet\IOFactory::identify($inputFileName, $testAgainstFormats);
```
...
try {
    ...
    $inputFileType = \PhpSpreadsheet\IOFactory::identify($inputFileName, [\PhpSpreadsheet\IOFactory::READER_XLS]);
...
`\PhpSpreadsheet\Reader\IReadFilter` interface, and contain a
...
/**  Define a Read Filter class implementing \PhpSpreadsheet\Reader\IReadFilter  */
...
/**  Define a Read Filter class implementing \PhpSpreadsheet\Reader\IReadFilter  */
class ChunkReadFilter implements \PhpSpreadsheet\Reader\IReadFilter
{
    private $startRow = 0;
    private $endRow   = 0;
...
`\PhpSpreadsheet\Cell\IValueBinder` interface. It must contain a
`bindValue()` method that accepts a `\PhpSpreadsheet\Cell\Cell` and a
