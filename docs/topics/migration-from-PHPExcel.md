...
```sh
composer require rector/rector alsar/PhpSpreadsheet --dev
...
// After
\PhpSpreadsheet\IOFactory::registerReader($type, $classname);
```
...
// Or alternatively
\PhpSpreadsheet\IOFactory::registerWriter('Pdf', \PhpSpreadsheet\Writer\Pdf\Mpdf::class);
...
In addition to that, `\PhpSpreadsheet::getCellCollection()` was renamed
to `\PhpSpreadsheet::getCoordinates()` and
`\PhpSpreadsheet::getCellCacheController()` to
`\PhpSpreadsheet::getCellCollection()` for clarity.
...
to a dedicated new class `\PhpSpreadsheet\Cell\Coordinate`. The methods are:
...
their original default values, see [that commit](https://github.com/alsar/PhpSpreadsheet/commit/033a4bdad56340795a5bf7ec3c8a2fde005cda24).
