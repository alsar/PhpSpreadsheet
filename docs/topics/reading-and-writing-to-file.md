...
implementations of `\PhpSpreadsheet\Reader\IReader` and
`\PhpSpreadsheet\Writer\IWriter`.

## \PhpSpreadsheet\IOFactory

The PhpSpreadsheet API offers multiple methods to create a
`\PhpSpreadsheet\Reader\IReader` or
`\PhpSpreadsheet\Writer\IWriter` instance:

Direct creation via `\PhpSpreadsheet\IOFactory`. All examples
underneath demonstrate the direct creation method. Note that you can
also use the `\PhpSpreadsheet\IOFactory` class to do this.

### Creating `\PhpSpreadsheet\Reader\IReader` using `\PhpSpreadsheet\IOFactory`
...
`\PhpSpreadsheet\Reader\IReader` distributed with
PhpSpreadsheet. If one of them can load the specified file name, the
file is loaded using that `\PhpSpreadsheet\Reader\IReader`.
Explicit mode requires you to specify which
`\PhpSpreadsheet\Reader\IReader` should be used.

You can create a `\PhpSpreadsheet\Reader\IReader` instance using
`\PhpSpreadsheet\IOFactory` in automatic file type resolving
mode using the following code sample:

```php
$spreadsheet = \PhpSpreadsheet\IOFactory::load("05featuredemo.xlsx");
```
...
```php
$reader = \PhpSpreadsheet\IOFactory::createReaderForFile("05featuredemo.xlsx");
$reader->setReadDataOnly(true);
$reader->load("05featuredemo.xlsx");
```

You can create a `\PhpSpreadsheet\Reader\IReader` instance using
`\PhpSpreadsheet\IOFactory` in explicit mode using the following
code sample:
...
### Creating `\PhpSpreadsheet\Writer\IWriter` using `\PhpSpreadsheet\IOFactory`

You can create a `\PhpSpreadsheet\Writer\IWriter` instance using
`\PhpSpreadsheet\IOFactory`:
...
`\PhpSpreadsheet\Reader\IReadFilter`. By default, all cells are
read using the `\PhpSpreadsheet\Reader\DefaultReadFilter`.
...
```php
$reader = new \PhpSpreadsheet\Reader\Xml();
...
```php
$reader = new \PhpSpreadsheet\Reader\Slk();
...
Note that `\PhpSpreadsheet\Reader\Csv` by default assumes that
...
```php
$reader = new \PhpSpreadsheet\Reader\Csv();
$encoding = \PhpSpreadsheet\Reader\Csv::guessEncoding('sample.csv');
// or, e.g. $encoding = \PhpSpreadsheet\Reader\Csv::guessEncoding(
...
```php
$reader = new \PhpSpreadsheet\Reader\Csv();
$reader->setInputEncoding(\PhpSpreadsheet\Reader\Csv::GUESS_ENCODING);
...
```php
function constructorCallback(\PhpSpreadsheet\Reader\Csv $reader): void
{
    $reader->setInputEncoding(\PhpSpreadsheet\Reader\Csv::GUESS_ENCODING);
...
\PhpSpreadsheet\Reader\Csv::setConstructorCallback('constructorCallback');
...
```php
\PhpSpreadsheet\Shared\StringHelper::setDecimalSeparator('.');
\PhpSpreadsheet\Shared\StringHelper::setThousandsSeparator(',');
```
...
```php
$reader = new \PhpSpreadsheet\Reader\Html();
...
Please note that `\PhpSpreadsheet\Writer\Html` only outputs the
...
an existing website. \PhpSpreadsheet\Writer\Html provides
...
See section `\PhpSpreadsheet\Writer\Csv` how to control the
appearance of these.
...
```php
$writer = \PhpSpreadsheet\IOFactory::createWriter($spreadsheet, 'Mpdf');
```
...
```php
$class = \PhpSpreadsheet\Writer\Pdf\Mpdf::class;
\PhpSpreadsheet\IOFactory::registerWriter('Pdf', $class);
...
```php
$writer = new \PhpSpreadsheet\Writer\Pdf\Mpdf($spreadsheet);
```
...
class My_Custom_TCPDF_Writer extends \PhpSpreadsheet\Writer\Pdf\Tcpdf
{
...
\PhpSpreadsheet\IOFactory::registerWriter('Pdf', MY_TCPDF_WRITER::class);
```
...
Please note that `\PhpSpreadsheet\Writer\Pdf` only outputs the
first worksheet by default.
...
```php
$reader = \PhpSpreadsheet\IOFactory::createReaderForFile("spreadsheetWithCharts.xlsx");
...
```php
$spreadsheet = \PhpSpreadsheet\IOFactory::load("spreadsheetWithCharts.xlsx", \PhpSpreadsheet\Reader\IReader::LOAD_WITH_CHARTS);
```
...
```php
$writer = IOFactory::createWriter($spreadsheet, 'Xlsx');
$writer->save('mySavedFileWithCharts.xlsx', \PhpSpreadsheet\Writer\IWriter::SAVE_WITH_CHARTS);
```
...
```php
$reader = \PhpSpreadsheet\IOFactory::createReaderForFile("myExampleFile.xlsx");
