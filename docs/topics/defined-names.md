...
// Define named ranges
$spreadsheet->addNamedRange( new \PhpSpreadsheet\NamedRange('TAX_RATE', $worksheet, '=$B$1') );
$spreadsheet->addNamedRange( new \PhpSpreadsheet\NamedRange('PRICE', $worksheet, '=$B$3') );
...
$worksheet = $spreadsheet->addSheet(new \PhpSpreadsheet\Worksheet\Worksheet($spreadsheet, 'Client Timesheet'));
...
foreach ($clients as $clientName => $workHours) {
    $worksheet = $spreadsheet->addSheet(new \PhpSpreadsheet\Worksheet\Worksheet($spreadsheet, $clientName));
