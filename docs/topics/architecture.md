...
pre-existing project with `composer require alsar/PhpSpreadsheet`.
...
`\PhpSpreadsheet\Reader\IReader` and
`\PhpSpreadsheet\Writer\IWriter` interface in a custom class.
